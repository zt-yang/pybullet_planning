...
class PDDLStreamEnv(PDDLEnv):
    ...
    def identify_missing_facts(self, action, verbose=True, use_original_domain=False):
    ...
    def step(self, action):
        ...
        info['action'] = action
        info['missing_actions'] = self.get_missing_actions(action, info_original, info)
    ...
    def get_missing_actions(self, next_action, info_original, info):
        ...
        if len(missing_preconditions) > 0:
            init = self.problems[0].initial_state
    ...
    def get_extended_plan(self, abstract_plan, init, verbose=True):
        ...
        for step in abstract_plan:
            ...
            updated_plan.extend(debug_info['missing_actions'])
